...
export class NewPlayPage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.player) {
      ...
      this.scenario
        .getStarting()
        .availableSpells.forEach((type: EntityType) => {
  ...
  onClickValidate(): void {
    ...
    if (this.scenario.getStarting().askForName && !this.name) {
      valid = false;
  ...
  onClickSpell(spell: Spell): void {
    if (this.spellIsSelected(spell)) {
      ...
      if (this.selectedSpells.length < this.scenario.getStarting().maxSpells) {
